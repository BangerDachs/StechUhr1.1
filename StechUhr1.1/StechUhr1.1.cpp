    ...
    do {
        ...
        if (exists && isValueInCSV(mitarbeiterliste, userInput)) {
            ...
            if (lastStatus == "anwesend") {
                cout << "Was möchtest du machen? abmelden | letzten 30 Tage anschauen und speichern: abmelden = 1 | 30 Tage = 2 | EXIT = 0 " << endl;
                getline(cin, status);
                if (status == "1") {
                ...
                if (status == "ja") {
                    ...
                    cout << "Möchtest du dir die letzten 30 Tage anschauen und speichern? ja/nein: ";
                    getline(cin, status);
                    if (status == "ja") {
                        string userFile = folder + userInput + ".csv";
                        displayLast30Days(userFile,userInput);
